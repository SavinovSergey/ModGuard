...
Метрики e2e latency, throughput и from_cache берутся из Postgres (tasks.created_at /
completed_at, task_items.from_cache). Throughput (Postgres) = items / (max(completed_at)−
min(created_at)). DATABASE_URL скрипта должен совпадать с тем, куда пишут API/воркер.
...
def _batch_async_payload(items: list[tuple[str, int]]) -> dict[str, Any]:
    return {
...
def _parse_batch_response(data: dict[str, Any], status_code: int) -> tuple[list[str], int]:
    tids = data.get("task_ids")
    if tids:
        return list(tids), status_code
...
def send_batch(api_url: str, items: list[tuple[str, int]], timeout: float = 120.0) -> tuple[list[str], int]:
    ...
    try:
        r = requests.post(url, json=_batch_async_payload(items), timeout=timeout)
        if r.status_code != 200:
            return [], r.status_code
        return _parse_batch_response(r.json(), r.status_code)
...
async def send_batch_async(
    ...
    url = f"{api_url.rstrip('/')}/api/v1/classify/batch-async"
    try:
        r = await client.post(url, json=_batch_async_payload(items))
        if r.status_code != 200:
            return [], r.status_code
        return _parse_batch_response(r.json(), r.status_code)
    except Exception as e:
...
async def fetch_task_results_async(
    client: httpx.AsyncClient,
    api_url: str,
    task_id: str,
...
def _apply_poll_outcome(
    ...
    """Обновляет состояние опроса; True — task ещё не в терминальном статусе."""
    if err and st == "error":
        errors[tid] = err
    elif err and st == "failed":
        errors[tid] = err
    elif err and st is None:
        return True
    if st:
        statuses[tid] = st
    if st == "completed":
        results_count[tid] = n_res
    elif st not in terminal:
        return True
    return False


async def _poll_one_task_async(
    client: httpx.AsyncClient,
    url_base: str,
    tid: str,
) -> tuple[str, str | None, int, str | None]:
    try:
        r = await client.get(f"{url_base}/{tid}")
        if r.status_code != 200:
            return tid, "error", 0, f"HTTP {r.status_code}"
        data = r.json()
        st = data.get("status", "")
        if st == "completed":
            return tid, st, len(data.get("results") or []), None
        if st == "failed":
            return tid, st, 0, data.get("error") or "unknown"
        return tid, st, 0, None
    except Exception as e:
...
async def poll_until_done_async(
    ...
    errors: dict[str, str] = {}
    terminal = frozenset({"completed", "failed", "error"})
    ...
    while time.perf_counter() - started < timeout:
        ...
        for tid, st, n_res, err in outcomes:
            if _apply_poll_outcome(tid, st, n_res, err, statuses, results_count, errors, terminal):
                still_pending = True
...
async def _send_prepared_batch(
    client: httpx.AsyncClient,
    api_url: str,
    prepared: dict[str, Any],
) -> dict[str, Any]:
    t0 = time.perf_counter()
    tids, status = await send_batch_async(client, api_url, prepared["items_pairs"])
    return {
        "dt": time.perf_counter() - t0,
        "has_dup": prepared["has_dup"],
        "tids": tids,
        "status": status,
        "prepared": prepared,
    }
...
async def send_prepared_batches_async(
    api_url: str,
...
async def run_chain_http_async(
    ...
    for sr in send_results:
        batch_send_times.append(sr["dt"])
        batch_send_hot.append(sr["has_dup"])
        prep = sr["prepared"]
        tids = sr["tids"]
        if tids:
            submitted.append(
            ...
            print(f"  [batch] HTTP {sr['status']}, items={prep['n_items']}")
    ...
    if unique_ids:
        ...
        poll_limits = httpx.Limits(max_connections=64, max_keepalive_connections=32)
        async with httpx.AsyncClient(timeout=poll_timeout, limits=poll_limits) as client:
            print(f"\nОтправлено батчей: {len(send_results)}, task_id: {len(unique_ids)}")
            print("Polling (async)...")
            summary = await poll_until_done_async(
                client, api_url, unique_ids, poll_interval, timeout
            )
            if fetch_quality:
                completed_tids = [
                ...
                if completed_tids:
                    ...
                    for tid, (st, res) in zip(completed_tids, outcomes):
                        if st == "completed" and res is not None:
...
def pg_throughput_for_tasks(task_ids: list[str]) -> dict[str, float | int] | None:
    """
    Throughput по Postgres: число task_items / (max(completed_at) - min(created_at))
    по выбранным task_id. Окно — от постановки первого батча в БД до завершения последнего.
    """
    if not _pg_available() or not task_ids:
        return None
    from app.core.db import get_db_connection
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    SELECT MIN(t.created_at), MAX(t.completed_at), COUNT(ti.item_index)
                    FROM tasks t
        ...
        if span <= 0 or n_items <= 0:
            return None
        return {
            "n_items": int(n_items),
            "span_sec": float(span),
            "throughput": float(n_items) / span,
        }
    except Exception as e:
        print(f"  [PG] throughput: {e}")
        return None
...
def run(
    ...
    submitted = chain["submitted"]
    all_task_ids = chain["all_task_ids"]
    unique_ids = chain["unique_ids"]
    ...
    if batch_send_times:
        ...
        print(f"  Отправка батча (все): wall={send_wall:.2f}s, avg POST={avg_send * 1000:.0f}ms  {_fmt_pcts(pcts)}")
    ...
    if total_results > 0 and summary["elapsed"] > 0:
        ...
        print(f"  Throughput (скрипт, wall): {total_results / wall:.0f} msg/sec (wall {wall:.1f}s)")
    ...
    if e2e_map:
        ...
        cold_tids = [
            tid for tid in unique_ids
            if tid in e2e_map and not tid_hot.get(tid, False)
        ]
        if cold_tids and hot_e2e:
            pg_cold = pg_throughput_for_tasks(cold_tids)
            if pg_cold:
                print(
                    f"  Throughput (Postgres, cold): {pg_cold['throughput']:.0f} msg/sec "
                    f"({pg_cold['n_items']} items / {pg_cold['span_sec']:.2f}s)"
                )
        elif cold_tids and not hot_e2e:
            # duplicate_ratio=0 — cold совпадает с «все»; не дублируем строку
            pass
...
def main():
    ...
    p.add_argument(
        ...
        type=int,
        default=32,
        help="Параллельных POST batch-async (0 — без лимита, все через gather)",
    )
    p.add_argument(
        "--send-timeout",
        type=float,
